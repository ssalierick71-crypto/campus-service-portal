...
@section('title', 'Edit — People — Campus Service Portal')

@section('content')
    <!-- Start: Page heading -->
    <div class="max-w-2xl">
        <h1 class="text-xl font-semibold mb-2">Edit person</h1>
        <p class="text-sm text-slate-500 mb-6">Change their role or correct a name. Passwords stay hidden.</p>
    </div>
    <!-- End: Page heading -->
    ...
    <form action="{{ route('admin.users.index') }}" method="get" class="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm max-w-2xl">
        <div class="p-6 space-y-5">
            <div><label class="block text-sm font-medium text-slate-600 mb-2">Full name</label><input class="input-field" name="name" value="Daniel Okello"></div>
            <div><label class="block text-sm font-medium text-slate-600 mb-2">Email</label><input type="email" class="input-field" name="email" value="daniel.okello@campus.ac.ug"></div>
